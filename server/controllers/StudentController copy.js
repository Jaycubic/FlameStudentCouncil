...
const getTotalStudentCount = async (req, res) => {
  try {
    ...
    const io = getIo();  // Already correct, kept as is
...
const getGenderBatchCount = async (req, res) => {
  try {
    ...
    const io = getIo();  // Fixed: Added this line
...
const getRCNames = async (req, res) => {
  try {
    const rcNames = await sequelize.query(
      `SELECT DISTINCT RCName FROM studentdata WHERE RCName IS NOT NULL ORDER BY RCName ASC`,
      { type: QueryTypes.SELECT }
    );
