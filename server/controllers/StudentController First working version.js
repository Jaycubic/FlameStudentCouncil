...
// Get all student data in terms of Student Info with search
const getStudentInfo = async (req, res) => {
  try {
    ...
    const cachedKey = `studentInfo:page:${page}:search:${search}`;
    const cachedData = await redisClient.get(cachedKey);
    if (cachedData) {
      console.log(`Serving studentInfo page ${page} with search "${search}" from Redis cache`);
      return res.json(JSON.parse(cachedData));
    }
    ...
    await redisClient.setEx(cachedKey, 600, JSON.stringify(students));
    const io = getIo();
    io.emit('studentInfoUpdated', { page, search, data: students });
    res.json(students);
  } catch (error) {
