...
const authController = {
  ...
  async login(req, res) {
    try {
      ...
      if (user.Role.name === 'admin') {
        ...
        // Non-admin users now require email verification before Google Sign-In
        const verificationCode = generateVerificationCode();
        const tokenExpires = new Date(Date.now() + 10 * 60 * 1000);
        const hashedCode = await bcrypt.hash(verificationCode, 10);
  ...
  async verifyCode(req, res) {
    ...
    try {
      ...
      if (user.Role.name === 'admin') {
        ...
        const is2FAEnabled = settings.admin_2fa_enabled;
        ...
        // Non-admin user: proceed to Google Sign-In after verification
  ...
  async resendVerificationCode(req, res) {
    ...
    try {
      ...
      const verificationCode = generateVerificationCode();
      const tokenExpires = new Date(Date.now() + 10 * 6080 * 1000);
      const hashedCode = await bcrypt.hash(verificationCode, 10);
  ...
  async googleCallback(req, res) {
    ...
    try {
      ...
      const mostly = await oauth2.userinfo.get();
