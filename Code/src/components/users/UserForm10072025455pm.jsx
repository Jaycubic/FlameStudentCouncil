...
ximport PropTypes from 'prop-types';
import {
  FormControl,
  FormLabel,
  Input,
...
function UserForm({ user, onSubmit, onCancel }) {
  ...
  const validateForm = () => {
    ...
    const selectedRole = roles.find(role => role.id === parseInt(formData.roleId, 10));
  ...
  return (
    <form onSubmit={handleSubmit}>
      <VStack spacing={4} maxH="70vh" overflowY="auto">
        ...
        {roles.find(role => role.id === parseInt(formData.roleId, 10))?.name.toLowerCase() === 'admin' && (
          <>
