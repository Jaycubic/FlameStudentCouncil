...
function StudentHouseTracking() {
  ...
  const [keyActionStudent, setKeyActionStudent] = useState(null);
  ...
  const getInOutBgColor = (inout, noOfDays) => {
    if (inout === 'OUT') {
      return 'orange.100';
    } else if (inout === 'IN' && noOfDays != null) {
      if (noOfDays === 0) {
        return 'green.100';
      } else if (noOfDays >= 1 && noOfDays <= 2) {
        return 'yellow.100';
      } else if (noOfDays >= 3 && noOfDays <= 5) {
        return 'orange.100';
      } else if (noOfDays > 5) {
        return 'red.100';
      }
  ...
  const handleKeyClick = async (student) => {
    setKeyActionStudent(student);
    try {
      const status = await keyService.getKeyStatus(student.StudentCvueNo);
      setKeyStatus(status);
      onKeyModalOpen();
    } catch (error) {
      toast({
  ...
  const handleIssueKey = async () => {
    try {
      toast({
        title: 'Issuing Key',
        description: 'Sending email and saving to database...',
        status: 'info',
        duration: null,
        isClosable: false,
        render: () => (
          <Box color="white" p={3} bg="blue.500" borderRadius="md">
            ...
            Sending email and saving to database...
          </Box>
        ),
      });
      await keyService.issueKey(keyActionStudent.StudentCvueNo);
      toast.closeAll();
      toast({
        title: 'Success',
        description: 'Key issued and email sent successfully',
        status: 'success',
        duration: 2000,
      });
      onKeyModalClose();
  ...
  const handleReturnKey = async () => {
    try {
      await keyService.returnKey(keyActionStudent.StudentCvueNo);
      toast({
        title: 'Success',
  ...
  return (
    <Box p={8}>
                      ...
                      <Tr key={student.id} _hover={{ bg: 'gray.50' }}>
                        ...
                        <Td borderColor={borderColor}>
                          <HStack spacing={2}>
                            ...
                            <Tooltip label="Manage key" hasArrow>
                              <IconButton
                                icon={<KeyIcon className="h-4 w-4" />}
                                variant="ghost"
                                colorScheme="green"
                                size="sm"
                                onClick={() => handleKeyClick(student)}
                              />
                            </Tooltip>
                          </HStack>
                        </Td>
                ...
                students.map(student => (
                  <Card key={student.id} bg={bgColor} w="100%" p={4}>
                    <VStack align="start" spacing={2}>
                      ...
                      <Text fontSize="sm" bg={getInOutBgColor(student.INOUT, student.NoOfDays)}>
      ...
      <Modal isOpen={isKeyModalOpen} onClose={onKeyModalClose}>
        <ModalOverlay />
        <ModalContent>
          <ModalHeader>Manage Key for {keyActionStudent?.StudentName}</ModalHeader>
          <ModalCloseButton />
          <ModalBody>
            {keyStatus?.status === 'issued' ? (
              <Text>You have already issued a key to this student on {new Date(keyStatus.issuedDate).toLocaleDateString()}. Would you like to return the key?</Text>
            ) : (
              <Text>Would you like to issue a key to this student?</Text>
            )}
          </ModalBody>
          <ModalFooter>
            <Button variant="outline" mr={3} onClick={onKeyModalClose}>
              Cancel
            </Button>
            {keyStatus?.status === 'issued' ? (
              <Button colorScheme="red" onClick={handleReturnKey}>
                Return Key
              </Button>
            ) : (
              <Button colorScheme="green" onClick={handleIssueKey}>
                Issue Key
              </Button>
            )}
          </ModalFooter>
