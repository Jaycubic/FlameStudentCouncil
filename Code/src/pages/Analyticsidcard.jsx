...
function StudentInfoTable() {
  ...
  const handleGeneratePDF = async () => {
    ...
    try {
      const response = await idCardService.generatePDF(selectedStudents);
                ...
                {isLoading ? (
                  ...
                  students.map(student => (
                    <Tr
                      key={student.id}
                      _hover={{ bg: 'gray.50' }}
                      bg={completedStudents.includes(student.id) ? 'green.50' : 'inherit'}
                    >
                      ...
                      <Td borderColor={borderColor}>
                        <Text fontSize="sm" color={textColor}>{student.House || 'N/A'}</Text>
                      </Td>
                      <Td borderColor={borderColor}>
            ...
            {modalStudent && (
              <Flex direction={{ base: 'column', md: 'row' }} align="center">
                ...
                <VStack align="start" spacing={2}>
                  ...
                  <Text color={textColor}>Student ID: {modalStudent.StudentCvueNo}</Text>
                  <Text color={textColor}>DOB: {formatDate(modalStudent.DOB)}</Text>
                  <Text color={textColor}>Batch: {modalStudent.Batch}</Text>
                  <Text color={textColor}>House: {modalStudent.House || 'N/A'}</Text>
                </VStack>
              </Flex>
            )}
          </ModalBody>
