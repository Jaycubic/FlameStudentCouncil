...
class AuthService {
  ...
  async login(email, password) {
    try {
      ...
      const response = await fetch('https://flameawards.in:8082/api/auth/login', {
        method: 'POST',
  ...
  async verifyCode(userId, code) {
    try {
      const deviceId = await this.getDeviceId();
      const response = await fetch('https://flameawards.in:8082/api/auth/verify-code', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  async verify2FA(userId, code) {
    try {
      const deviceId = await this.getDeviceId();
      const response = await fetch('https://flameawards.in:8082/api/auth/verify-2fa', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ userId, code, deviceId }),
        credentials: 'include',
  ...
  async resendVerificationCode(userId) {
    try {
      const deviceId = await this.getDeviceId();
      const response = await fetch('https://flameawards.in:8082/api/auth/resend-verification-code', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  async googleSignIn() {
    try {
      const deviceId = await this.getDeviceId();
      const response = await fetch(`https://flameawards.in:8082/api/auth/google?deviceId=${encodeURIComponent(deviceId)}`, {
        method: 'GET',
        credentials: 'include',
      });
      if (!response.ok) {
  ...
  async initiateGoogleSignIn(email) {
    try {
      const response = await fetch('https://flameawards.in:8082/api/auth/initiate-google-signin', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  async verifyGoogleSignInCode(userId, code) {
    try {
      const deviceId = await this.getDeviceId();
      const response = await fetch('https://flameawards.in:8082/api/auth/verify-google-signin-code', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ userId, code, deviceId }),
        credentials: 'include',
  ...
  async refresh() {
    try {
      const deviceId = await this.getDeviceId();
      const response = await fetch('https://flameawards.in:8082/api/auth/refresh', {
        method: 'POST',
        headers: {
